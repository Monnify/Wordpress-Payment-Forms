<?php
/**
 * The Submissions admin page and CSV export handler.
 *
...
class Submissions {
	...
	public function __construct() {
		...
		add_action( 'admin_post_mff_monnify_export_excel', [ $this, 'export_excel' ] );
	}
	...
	public function register_submissions_page() {
		add_submenu_page( 'edit.php?post_type=monnify_form', esc_html__( 'Submissions', 'payment-forms-for-monnify' ), esc_html__( 'Submissions', 'payment-forms-for-monnify' ), 'administrator', 'submissions', [ $this, 'output_submissions_page' ] );
		remove_submenu_page( 'edit.php?post_type=monnify_form', 'submissions' );
	}
	...
	public function output_submissions_page() {
		...
		if ( 'monnify_form' === get_post_type( $form ) ) {
			$payments_table = $this->get_payments_list_table();
			$data           = $payments_table->prepare_items();
			?>
			<div class="info-panel">
				<div class="info-panel-content">
				<h1 style="margin: 0px;"><?php echo esc_html( get_the_title( $form ) ); ?> <?php esc_html_e( 'Payments', 'payment-forms-for-monnify' ); ?></h1>
					<p class="about-description">
						<?php esc_html_e( 'All payments made for this form', 'payment-forms-for-monnify' ); ?>
					</p>
					<?php if ( $data > 0 ) { ?>
						<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
							<input type="hidden" name="action" value="mff_monnify_export_excel">
							<input type="hidden" name="form_id" value="<?php echo esc_attr( $form_id ); ?>">
							<?php wp_nonce_field( 'mff_monnify_export_excel' ); ?>
							<button type="submit" class="button button-primary button-hero load-customize"><?php esc_html_e( 'Export Data to Excel', 'payment-forms-for-monnify' ); ?></button>
						</form>
					<?php } ?>
					<br />
					<br />
	 ...
	 *
	 * @return \monnify\payment_forms\Payments_List_Table
	 */
	public function get_payments_list_table() {
		if ( ! class_exists( 'WP_List_Table' ) ) {
			include_once ABSPATH . 'wp-admin/includes/class-wp-list-table.php';
		}
		include_once MFF_MONNIFY_PLUGIN_PATH . 'includes/classes/class-payments-list-table.php';
		return new \monnify\payment_forms\Payments_List_Table();
	}
	...
	public function export_excel() {
		check_admin_referer( 'mff_monnify_export_excel' );
