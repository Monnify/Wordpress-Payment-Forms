<?php
/**
 * The Receipt Email sent to the site owner after a successful payment.
 *
...
class Email_Receipt_Owner extends Email {
	...
	public function __construct() {
		add_action( 'mff_monnify_send_receipt_owner', [ $this, 'send_receipt_owner' ], 11, 7 );
	}
	...
	function send_receipt_owner( $form_id, $currency, $amount, $name, $email, $code, $metadata ) {
		...
		// Custom Values
		$this->subject     = esc_html__( 'You just received a payment', 'payment-forms-for-monnify' );
												...
												<tbody>
													<tr>
														<td class="column_cell font_default" align="center" valign="top" style="padding:16px;font-family:Helvetica,Arial,sans-serif;font-size:15px;text-align:center;vertical-align:top;color:#888">
															...
															<h5 style="font-family:Helvetica,Arial,sans-serif;margin-left:0;margin-right:0;margin-top:16px;margin-bottom:8px;padding:0;font-size:18px;line-height:26px;font-weight:bold;color:#383d42"><?php esc_html_e( 'You just received a payment', 'payment-forms-for-monnify' ); ?></h5>
														</td>
													</tr>
												</tbody>
											</table>
																...
																<tbody>
																	...
																	<tr>
																		<td class="font_default" style="padding:3px 7px;font-family:Helvetica,Arial,sans-serif;font-size:10px;font-weight:bold;text-transform:uppercase;letter-spacing:2px;-webkit-border-radius:2px;border-radius:2px;white-space:nowrap;background-color:#666;color:#fff">
																			<?php esc_html_e( 'Payment Details', 'payment-forms-for-monnify' ); ?>
																		</td>
																	</tr>
																</tbody>
															</table>
															<p style="font-family:Helvetica,Arial,sans-serif;font-size:15px;line-height:23px;margin-top:8px;margin-bottom:16px">
																<?php esc_html_e( 'Amount', 'payment-forms-for-monnify' ); ?> <strong>: <?php echo esc_html( $this->currency ) . ' ' . number_format( $this->amount ); ?></strong><br>
																<?php esc_html_e( 'Email', 'payment-forms-for-monnify' ); ?> <strong>: <?php echo esc_html( $this->html_email ); ?></strong><br>
																<?php
																	...
																	if ( is_array( $new ) && array_key_exists( '0', $new ) ) {
																		foreach ( $new as $key => $item ) {
																			if ( 'text' === $item->type ) {
																				...
																				echo esc_html( $item->display_name ) . '<strong> : <a target="_blank" href="' . esc_url( $item->value ) . '">' . esc_html__( 'link', 'payment-forms-for-monnify' ) . '</a></strong><br>';
																			}
																		}
																...
																?>
																<?php esc_html_e( 'Transaction reference', 'payment-forms-for-monnify' ); ?>: <strong><?php echo esc_html( $this->code ); ?></strong><br>
															</p>
														</td>
													</tr>
												</tbody>
												...
												<tbody>
													<tr>
														<td class="column_cell font_default" align="center" valign="top" style="padding:16px 16px 0;font-family:Helvetica,Arial,sans-serif;font-size:15px;text-align:center;vertical-align:top;color:#888">
															<small style="font-size:86%;font-weight:normal"><strong><?php echo esc_html__( 'Notice', 'payment-forms-for-monnify' ); ?></strong><br>
															<?php echo esc_html__( 'You\'re getting this email because someone made a payment of', 'payment-forms-for-monnify' ); ?> <?php echo esc_html( $this->currency . ' ' . number_format( $this->amount ) ); ?> <?php echo esc_html__( 'to', 'payment-forms-for-monnify' ); ?> <a href="<?php echo esc_html( get_bloginfo( 'url' ) ); ?>" style="display:inline-block;text-decoration:none;font-family:Helvetica,Arial,sans-serif;color:#2f68b4"><?php echo esc_html( get_option( 'blogname' ) ); ?></a>.</small>
														</td>
													</tr>
												</tbody>
											</table>
