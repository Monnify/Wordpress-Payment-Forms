...
class Field_Shortcodes {
	...
	public function text_field( $atts ) {
		$atts = shortcode_atts(
			array(
				'name'     => esc_html__( 'Title', 'payment-forms-for-monnify' ),
				'required' => '0',
			),
			$atts,
			'text'
		...
		$name     = sanitize_text_field( sprintf( esc_attr__( 'Enter %s', 'payment-forms-for-monnify' ), $atts['name'] ) );
	...
	/**
	 * Generates the "checkbox" input field.
	 *
	 * @param array $atts
	 * @return string
	 */
	...
	public function input_field( $atts ) {
		...
		$code .= esc_html__( 'Browse', 'payment-forms-for-monnify' );
		...
		$code .= '</div>';
		$code .= '<input type="text" id="' . esc_attr( $textInputId ) . '" readonly="" placeholder="' . esc_attr__( 'No file selected', 'payment-forms-for-monnify' ) . '">';
		$code .= '</div></div>';
